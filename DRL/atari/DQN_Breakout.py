...
from DRL.utils.wrapper import BreakoutWrapper
from DRL.utils.memory import ReplayMemory
from DRL.utils.network import Conv2d_nn
...
PATH_LOG = 'DRL/atari/test1/log.txt'
...
class DQN_Breakout():
	...
	def __init__(self,
		...
		# Gym environnement Breakout
		self.env = BreakoutWrapper('BreakoutNoFrameskip-v4')
		...
		self.q_nn = Conv2d_nn(in_dim=self.env.observation_space.shape[0], 
		...
		#									lr=learning_rate)
		self.__optimizer =  torch.optim.RMSprop(self.q_nn.parameters(),
											lr=learning_rate,
											eps=0.001,
											alpha=0.95,
											momentum=0.95)
	...
	def play(self, display=True):
		...
		self.log('## Done, mean={} '.format(mean))
...
hyperparams = []
hyperparams.append([0.00025, 64, 0.99, 32, 10000])




for p in hyperparams:
	agent = DQN_Breakout(p[0], p[1], p[2], p[3], p[4])
	agent.run()
