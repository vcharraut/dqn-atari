...
class ReplayMemory():
	...
	def __init__(self, config):
		...
		self.next_states = torch.empty((self.batch_size, 4, 84, 84)).to(self.device)
	...
	def sample(self):
		...
		self.states = torch.empty((self.batch_size, 4, 84, 84)).to(self.device)
		self.next_states = torch.empty((self.batch_size, 4, 84, 84)).to(self.device)
		self.actions = torch.empty(self.batch_size).to(self.device).to(dtype=torch.long)
		self.rewards = torch.empty(self.batch_size).to(self.device)
		self.terminals = torch.empty(self.batch_size).to(self.device)
...
class PrioritizedReplayMemory():
	...
	def __init__(self, config):
		...
		self.priority_weight = config.prior_samp
		self.priority_exponent = config.prior_expo
