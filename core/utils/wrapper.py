...
class EpisodicLifeEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if lives < self.lives and lives > 0:
            # for Qbert sometimes we stay in lives == 0 condition
            # for a few framesso it's important to keep lives > 0,
            # so that we only reset once
...
class LazyFrames(object):
    def __init__(self, frames):
        ...
        It exists purely to optimize memory usage which
                can be huge for DQN's 1M frames replaybuffers.
